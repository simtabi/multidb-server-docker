...
# of the way, so `docker run ... db-toolkit-cli psql ...` behaves exactly like
# running psql.
...
if [[ -t 1 ]] && [[ "${1:-bash}" == "bash" ]]; then
    cat <<'BANNER'
db-toolkit-cli — clients only, no server here.

  psql / pg_dump / pg_dumpall / pg_restore   PostgreSQL
  mariadb / mariadb-dump (also mysql / mysqldump)
  zstd, rclone, openssl, jq

Engines are reachable by service name on the dbtk network, for example:
