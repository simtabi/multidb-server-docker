...
**D-46 — MySQL does not claim PITR, on evidence rather than preference.** Binary logging works for MySQL — ROW format, own volume, retention, `sync_binlog=1` — so the log is written and can be shipped off-site. What is missing is a tool to read it back, and five routes were tried: the official `mysql:8.4` image is `mysql-community-server-minimal` on Oracle Linux 9 and ships no `mysqlbinlog`; the only `mysqlbinlog` in its enabled repos is the 8.0 client, whose files **conflict** with the 8.4 server package; `mysql-community-client` is not in any enabled repo; MySQL Shell is present but at 8.4.10, and `util.dumpBinlogs`/`loadBinlogs` arrived in Shell 9.2; and third-party images carrying the binary could not be pulled here. A binary log nothing can replay is an audit trail, not a recovery path, so `engines/mysql/engine.conf` records `DBTK_ENGINE_PITR_CAPABLE=false` with that reasoning. MariaDB's image is Ubuntu and ships `mariadb-binlog`, so it claims the capability and proves it.
