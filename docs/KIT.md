# db-toolkit execution kit

How to take db-toolkit-design.md from spec to running, tested software with a self-healing build loop. The principle behind everything here: an agent can only self-heal what a check can catch, so every task ends in an executable check and nothing is "done" until its check passes.
...
- The spec: db-toolkit-design.md
- A GitHub repo (CI is the neutral judge for the cross-platform matrix your laptop can't run)

## 1. One-time setup

```
mkdir db-toolkit && cd db-toolkit
git init
mkdir docs && cp ~/Downloads/db-toolkit-design.md docs/SPEC.md
...
```markdown
# db-toolkit build rules

You are building db-toolkit per docs/SPEC.md. That file is the contract. Read it before any work.
