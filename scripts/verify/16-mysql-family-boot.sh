...
check_engine() {
    ...
    name="mdb-verify-$engine-$$"
    track_container "$name"

    docker run -d --name "$name" -e "$pw_env=mdb-throwaway-verify" "$img" >/dev/null \
        || vfail "$engine failed to start with only $pw_env set"

    wait_ready 90 "$engine to accept connections" \
        docker exec "$name" "$client" -uroot -pmdb-throwaway-verify -e "SELECT 1"

    # Baked defaults from SPEC section 6 must actually be in effect.
    local charset collation sql_mode
    charset="$(docker exec "$name" "$client" -uroot -pmdb-throwaway-verify -N -B \
    ...
    collation="$(docker exec "$name" "$client" -uroot -pmdb-throwaway-verify -N -B \
        -e "SELECT @@collation_server" 2>/dev/null | tr -d ' \r')"
    [[ "$collation" == utf8mb4_unicode_ci* ]] \
        || vfail "$engine collation_server is '$collation', expected utf8mb4_unicode_ci"

    sql_mode="$(docker exec "$name" "$client" -uroot -pmdb-throwaway-verify -N -B \
...
docker exec "$mysql_c" mysql -uroot -pmdb-throwaway-verify -e "SELECT 1" >/dev/null 2>&1 \
    || vfail "MySQL stopped responding once MariaDB was up"
docker exec "$mariadb_c" mariadb -uroot -pmdb-throwaway-verify -e "SELECT 1" >/dev/null 2>&1 \
