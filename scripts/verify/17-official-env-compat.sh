...
ran="$(docker exec -e PGPASSWORD=dbtk-throwaway-pgfile "$name" \
...
compat_mysql_family() {
    ...
    docker run -d --name "$name" \
        ...
        -e "${prefix}_PASSWORD=dbtk-throwaway-user" \
    ...
    local ran
    ran="$(docker exec "$name" "$client" -ucustom_user -pdbtk-throwaway-user custom_db -N -B \
