...
pooler_img="$(grep -oE "^DBTK_ENGINE_POOLER_IMAGE='[^']+'" engines/postgres/engine.conf \
              | sed "s/^DBTK_ENGINE_POOLER_IMAGE='//; s/'$//")"
[[ -n "$pooler_img" ]] || vfail "engines/postgres/engine.conf declares no pooler image"

net="dbtk-verify-pool-net-$$"
pg="dbtk-verify-pool-pg-$$"
bouncer="dbtk-verify-pool-bouncer-$$"
secrets="$(mktemp -d)"
...
docker run -d --name "$pg" --network "$net" --network-alias pg \
    -e POSTGRES_PASSWORD=dbtk-throwaway-super \
    -e DBTK_PG_DATABASES="poolapp:poolapp:dbtk-throwaway-app" \
...
wait_ready 60 "the pooler to accept connections" \
    docker run --rm --network "$net" -e PGPASSWORD=dbtk-throwaway-app \
        --entrypoint psql "$img" \
        -h "$bouncer" -p 6432 -U poolapp -d poolapp -qtAX -c 'SELECT 1'

# 1. An application user connects through the pooler.
got="$(via_pooler dbtk-throwaway-app poolapp poolapp 'SELECT 42')"
...
if docker exec "$bouncer" sh -c 'cat /etc/pgbouncer/userlist.txt 2>/dev/null' \
     | grep -q 'dbtk-throwaway-app'; then
...
for _ in 1 2 3 4 5 6 7 8; do
    docker run --rm -d --network "$net" -e PGPASSWORD=dbtk-throwaway-app \
        --entrypoint psql "$img" \
...
if via_pooler dbtk-throwaway-wrong-password poolapp poolapp 'SELECT 1' >/dev/null 2>&1; then
