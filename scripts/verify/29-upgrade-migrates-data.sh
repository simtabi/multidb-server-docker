...
project="${COMPOSE_PROJECT_NAME:-dbtk}"
...
track_volume "$vol16"
track_volume "$vol17"
add_cleanup "rm -rf '$DBTK_ROOT/backups/upgrade'"

seed="dbtk-verify-upg-seed-$$"
track_container "$seed"

docker volume create "$vol16" >/dev/null
docker run -d --name "$seed" -e POSTGRES_PASSWORD=dbtk-throwaway-verify \
...
CONFIRM=yes make upgrade ENGINE=pg FROM=16 TO=17 >/tmp/dbtk-upgrade.log 2>&1 \
    || { tail -15 /tmp/dbtk-upgrade.log >&2; vfail "make upgrade failed"; }
vinfo "upgrade completed"

# --- the data arrived on the new major --------------------------------------
check17="dbtk-verify-upg-17-$$"
track_container "$check17"
docker run -d --name "$check17" -e POSTGRES_PASSWORD=dbtk-throwaway-verify \
...
check16="dbtk-verify-upg-16-$$"
track_container "$check16"
docker run -d --name "$check16" -e POSTGRES_PASSWORD=dbtk-throwaway-verify \
    -v "$vol16:/var/lib/postgresql/data" "$old_img" >/dev/null
