...
need_file "$DBTK_ROOT/IMAGE-PROVENANCE.md"
...
while IFS= read -r ref; do
    ...
    if grep -qF "$repo" "$DBTK_ROOT/IMAGE-PROVENANCE.md"; then
    ...
    grep -rhoE '(^|[[:space:]])image:[[:space:]]*[A-Za-z0-9][A-Za-z0-9./_-]*(:[A-Za-z0-9._-]+)?(@sha256:[a-f0-9]+)?' \
        "$DBTK_ROOT"/docker-compose.yml "$DBTK_ROOT"/compose.*.yml 2>/dev/null \
        | sed -E 's/.*image:[[:space:]]*//'
    grep -hoE '^[a-z0-9]+[[:space:]]+[^[:space:]]+[[:space:]]+[^[:space:]]+' "$DBTK_ROOT/images/bases.tsv" 2>/dev/null \
        | awk '{print $3}'
    grep -hoE "_IMAGE='[^']+'" "$DBTK_ROOT"/engines/*/engine.conf 2>/dev/null \
        | sed -E "s/.*='//; s/'$//"
)
