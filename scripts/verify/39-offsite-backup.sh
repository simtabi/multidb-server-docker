...
#
# This check exists because the setting was present and inert. DBTK_S3_BUCKET
...
for var in DBTK_S3_KEY_ID_FILE DBTK_S3_KEY_SECRET_FILE DBTK_S3_HOST DBTK_S3_REGION; do
    grep -q "$var" "$DBTK_ROOT/scripts/backup" \
...
grep -q 'RCLONE_S3_ACCESS_KEY_ID' "$DBTK_ROOT/scripts/backup" \
...
if grep -qE 'rclone copy.*\|\|[[:space:]]*log' "$DBTK_ROOT/scripts/backup"; then
...
grep -q 'the off-site push FAILED' "$DBTK_ROOT/scripts/backup" \
...
if grep -qE '^DBTK_S3_(KEY_ID|KEY_SECRET)=' "$DBTK_ROOT/.env.example"; then
