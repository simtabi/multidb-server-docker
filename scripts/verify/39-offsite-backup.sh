...
#
# This check exists because the setting was present and inert. MDB_S3_BUCKET
...
for var in MDB_S3_KEY_ID_FILE MDB_S3_KEY_SECRET_FILE MDB_S3_HOST MDB_S3_REGION; do
    grep -q "$var" "$MDB_ROOT/scripts/backup" \
...
grep -q 'RCLONE_S3_ACCESS_KEY_ID' "$MDB_ROOT/scripts/backup" \
...
if grep -qE 'rclone copy.*\|\|[[:space:]]*log' "$MDB_ROOT/scripts/backup"; then
...
grep -q 'the off-site push FAILED' "$MDB_ROOT/scripts/backup" \
...
if grep -qE '^MDB_S3_(KEY_ID|KEY_SECRET)=' "$MDB_ROOT/.env.example"; then
